...
class AwardTests: BaseTestCase {
    ...
    /// Looping through each award to check whether that award has already been earned
    func testNoAward() throws {
        for award in awards {
            XCTAssertFalse(dataController.hasEarned(award: award), "New user should have no award")
        }
    }
...
//        try dataController.createSampleData()
    ...
    func testAddingItems() throws {
        ...
        for (count, value) in values.enumerated() {
            ...
            XCTAssertEqual(matches.count, count + 1, "Adding \(value) item should unlock \(count + 1) awards")
